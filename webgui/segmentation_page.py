...
from .util import ROOT_DIR, get_filename, list_sessions
...
def get_segmentation(videoname, params):
    ...
    for k in segmented:
        filename = get_filename(videoname, params, 'segmentation_%s' % k, 'png')
...
@segmentation_page.route('/segmentation_image/<videoname>/<image_type>', methods=['GET'])
def segmentation_image(videoname, image_type):
    ...
    if request.args.get('save', '0') == '1':
        ...
        session_save(videoname, 'displayable',
                     get_filename(videoname, segmentation_params, 'segmentation_displayable', 'png'))
...
def upload_file():
    ...
    if file and check_extension(file.filename, current_app.config['ALLOWED_EXTENSIONS']):
