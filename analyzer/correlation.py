...
def correlate_activities(activities, exposure_time):
    ...
    min_index = n_frames // 2 - n_shifts
    max_index = n_frames // 2 + n_shifts

    correlations = numpy.ndarray((n_neurons, n_neurons, 2 * n_shifts),
                                 dtype='float')

    def calc_one_correlation(i, j):
        for k in range(0, n_shifts):
            correlated = numpy.correlate(activities[j], activities[i], 'same')
            correlations[i, j, :] = correlated[min_index: max_index] / n_frames
